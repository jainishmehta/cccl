...
def scan_device(d_input, d_output, num_items, op, h_init, force_inclusive, stream=None):
    ...
    if get_bytes is not None and compute is not None:
        temp_storage_bytes = int(
            get_bytes(d_input, d_output, num_items, init_value=h_init, op=op, stream=stream)
        )
        d_temp = cp.empty(temp_storage_bytes if temp_storage_bytes > 0 else 0, dtype=np.uint8)
        compute(d_temp, d_input, d_output, num_items, init_value=h_init, op=op, stream=stream)
        return

    temp_storage_bytes = int(scanner(None, d_input, d_output, op, num_items, h_init, stream))
    d_temp = cp.empty(temp_storage_bytes if temp_storage_bytes > 0 else 0, dtype=np.uint8)
    scanner(d_temp, d_input, d_output, op, num_items, h_init, stream)
...
def test_exclusive_scan_well_known_plus():
    ...
    scan_device(d_input, d_output, d_input.size, OpKind.PLUS, h_init, False)
...
def test_inclusive_scan_well_known_plus(monkeypatch):
    ...
    scan_device(d_input, d_output, d_input.size, OpKind.PLUS, h_init, True)
...
def test_exclusive_scan_well_known_maximum():
    ...
    scan_device(d_input, d_output, d_input.size, OpKind.MAXIMUM, h_init, False)
...
def test_scan_transform_output_iterator(floating_array):
    ...
    scan_device(d_input, d_out_it, d_input.size, OpKind.PLUS, h_init, True)
...
def test_exclusive_scan_max():
    ...
    scan_device(d_input, d_output, d_input.size, max_op, h_init, False)
...
def test_inclusive_scan_add():
    ...
    scan_device(d_input, d_output, d_input.size, add_op, h_init, True)
...
def test_reverse_input_iterator(monkeypatch):
    ...
    scan_device(reverse_it, d_output, len(d_input), add_op, h_init, True)
...
def test_reverse_output_iterator():
    ...
    scan_device(d_input, reverse_it, len(d_input), add_op, h_init, True)
...
def test_inclusive_scan_with_lambda():
    ...
    scan_device(d_input, d_output, len(d_input), lambda a, b: a + b, h_init, True)
