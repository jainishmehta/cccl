...
def _segmented_reduce_run(
    d_in, d_out, d_start_offsets, d_end_offsets, op, h_init, num_segments, stream=None
):
    reducer = cuda.compute.make_segmented_reduce(
        d_in, d_out, d_start_offsets, d_end_offsets, op, h_init
    )
...
def test_segmented_reduce_well_known_plus(monkeypatch):
    ...
    _segmented_reduce_run(d_input, d_output, d_starts, d_ends, OpKind.PLUS, h_init, 3)
