...
def _run_reduce_multistep(alg, d_in, d_out, num_items, h_init, op):
    stream = None
    ...
    if get_bytes is not None and compute is not None:
        temp_storage_bytes = int(
            get_bytes(d_in, d_out, num_items, h_init=h_init, op=op, stream=stream)
        )
    ...
    try:
        temp_storage_bytes = int(alg(None, d_in, d_out, op, num_items, h_init, stream))
    except TypeError:
        temp_storage_bytes = int(alg(None, d_in, d_out, op, num_items, h_init))
    temp_storage = cp.empty(
        temp_storage_bytes if temp_storage_bytes > 0 else 0, dtype=np.uint8
    )
    try:
        alg(temp_storage, d_in, d_out, op, num_items, h_init, stream)
    except TypeError:
        alg(temp_storage, d_in, d_out, op, num_items, h_init)
...
def reduce_pointer(input_array, build_only):
    ...
    if not build_only:
        _run_reduce_multistep(alg, input_array, res, size, h_init, OpKind.PLUS)
...
def reduce_pointer_custom_op(input_array, build_only):
    ...
    if not build_only:
        _run_reduce_multistep(alg, input_array, res, size, h_init, my_add)
...
def reduce_iterator(inp, size, build_only):
    ...
    if not build_only:
        _run_reduce_multistep(alg, inp, res, size, h_init, my_add)
...
def reduce_pointer_lambda(input_array, build_only):
    ...
    if not build_only:
        _run_reduce_multistep(alg, input_array, res, size, h_init, op)
