...
def _run_select_multistep(selector, d_in, d_out, d_num_selected, num_items, cond):
    ...
    if get_bytes is not None and compute is not None:
        temp_bytes = int(
            get_bytes(d_in, d_out, d_num_selected, num_items, cond=cond)
        )
        temp_storage = cp.empty(temp_bytes if temp_bytes > 0 else 0, dtype=np.uint8)
        compute(temp_storage, d_in, d_out, d_num_selected, num_items, cond=cond)
        return

    try:
        temp_bytes = int(selector(None, d_in, d_out, d_num_selected, cond, num_items))
        temp_storage = cp.empty(temp_bytes if temp_bytes > 0 else 0, dtype=np.uint8)
        selector(temp_storage, d_in, d_out, d_num_selected, cond, num_items)
    except TypeError:
        selector(d_in, d_out, d_num_selected, cond, num_items)
...
def select_pointer(inp, out, num_selected, build_only):
    ...
    if not build_only:
        _run_select_multistep(selector, inp, out, num_selected, size, even_op)
...
def select_iterator(size, d_in, out, num_selected, build_only):
    ...
    if not build_only:
        _run_select_multistep(selector, d_in_iter, out, num_selected, size, less_than_50)
...
def select_struct(inp, out, num_selected, build_only):
    ...
    if not build_only:
        _run_select_multistep(
            selector, inp, out, num_selected, size, in_first_quadrant
        )
...
def select_stateful(inp, out, num_selected, threshold_state, build_only):
    ...
    if not build_only:
        _run_select_multistep(selector, inp, out, num_selected, size, threshold_select)
