...
def _run_scan_multistep(alg, d_in, d_out, num_items, init_value, op):
    get_bytes = getattr(alg, "get_temp_storage_bytes", None)
    compute = getattr(alg, "compute", None)
    if get_bytes is not None and compute is not None:
        try:
            temp_storage_bytes = int(
                get_bytes(d_in, d_out, num_items, init_value=init_value, op=op)
            )
        except TypeError:
            temp_storage_bytes = int(get_bytes(d_in, d_out, num_items, init_value))
        temp_storage = cp.empty(
            temp_storage_bytes if temp_storage_bytes > 0 else 0, dtype=np.uint8
        )
        try:
            compute(temp_storage, d_in, d_out, num_items, init_value=init_value, op=op)
        except TypeError:
            compute(temp_storage, d_in, d_out, num_items, init_value)
        return

    temp_storage_bytes = int(alg(None, d_in, d_out, op, num_items, init_value))
    temp_storage = cp.empty(
        temp_storage_bytes if temp_storage_bytes > 0 else 0, dtype=np.uint8
    )
    alg(temp_storage, d_in, d_out, op, num_items, init_value)
...
def scan_pointer(input_array, build_only, scan_type):
    ...
    if not build_only:
        _run_scan_multistep(alg, input_array, res, size, h_init, OpKind.PLUS)
...
def scan_pointer_custom_op(input_array, build_only, scan_type):
    ...
    if not build_only:
        _run_scan_multistep(alg, input_array, res, size, h_init, my_add)
...
def scan_iterator(inp, size, build_only, scan_type):
    ...
    if not build_only:
        _run_scan_multistep(alg, inp, res, size, h_init, OpKind.PLUS)
