...
def _run_reduce_multistep(alg, d_in, d_out, num_items, h_init, op):
    ...
    if get_bytes is not None and compute is not None:
        temp_storage_bytes = int(
            get_bytes(d_in, d_out, num_items, h_init=h_init, op=op)
        )
        temp_storage = cp.empty(
            temp_storage_bytes if temp_storage_bytes > 0 else 0, dtype=np.uint8
        )
        compute(temp_storage, d_in, d_out, num_items, h_init=h_init, op=op)
        return
...
def _run_binary_transform_multistep(alg, d_in1, d_in2, d_out, op, num_items):
    get_bytes = getattr(alg, "get_temp_storage_bytes", None)
    compute = getattr(alg, "compute", None)
    if get_bytes is not None and compute is not None:
    ...
    alg(d_in1, d_in2, d_out, op, num_items)
...
def reduce_zip_array(input_array, build_only):
    ...
    if not build_only:
        _run_reduce_multistep(alg, zip_iter, res, size, h_init, my_add)
