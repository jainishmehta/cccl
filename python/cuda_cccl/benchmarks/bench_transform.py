...
def _run_unary_transform_multistep(transform, d_in, d_out, op, num_items):
    get_bytes = getattr(transform, "get_temp_storage_bytes", None)
    compute = getattr(transform, "compute", None)
    if get_bytes is not None and compute is not None:
        temp_storage_bytes = int(get_bytes(d_in, d_out, op, num_items))
        temp_storage = cp.empty(
            temp_storage_bytes if temp_storage_bytes > 0 else 0, dtype=np.uint8
        )
        compute(temp_storage, d_in, d_out, op, num_items)
        return
    transform(d_in, d_out, op, num_items)


def _run_binary_transform_multistep(transform, d_in1, d_in2, d_out, op, num_items):
    get_bytes = getattr(transform, "get_temp_storage_bytes", None)
    compute = getattr(transform, "compute", None)
    if get_bytes is not None and compute is not None:
        temp_storage_bytes = int(get_bytes(d_in1, d_in2, d_out, op, num_items))
        temp_storage = cp.empty(
            temp_storage_bytes if temp_storage_bytes > 0 else 0, dtype=np.uint8
        )
        compute(temp_storage, d_in1, d_in2, d_out, op, num_items)
        return
    transform(d_in1, d_in2, d_out, op, num_items)
...
def unary_transform_pointer(inp, out, build_only):
    ...
    if not build_only:
        _run_unary_transform_multistep(transform, inp, out, op, size)
...
def unary_transform_iterator(size, out, build_only):
    ...
    if not build_only:
        _run_unary_transform_multistep(transform, d_in, out, op, size)
...
def binary_transform_pointer(inp1, inp2, out, build_only):
    ...
    if not build_only:
        _run_binary_transform_multistep(transform, inp1, inp2, out, OpKind.PLUS, size)
...
def binary_transform_pointer_custom_op(inp1, inp2, out, build_only):
    ...
    if not build_only:
        _run_binary_transform_multistep(transform, inp1, inp2, out, op, size)
...
def binary_transform_iterator(size, out, build_only):
    ...
    if not build_only:
        _run_binary_transform_multistep(
